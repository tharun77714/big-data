...
**PulsePrice** is a high-performance, distributed Big Data application that implements dynamic, demand-based surge pricing for e-commerce, running entirely on a single Ubuntu machine.
...
Our system follows a professional, enterprise-grade data pipeline from acquisition to real-time analytics, running entirely on one Ubuntu machine.
...
1.  **Ingestion (Velocity)**: Raw user events are generated by the Click Simulator and buffered in **Kafka** on `localhost`. This decouples the simulation from the processing.
...
3.  **Persistence (Integrity)**: Final pricing decisions are saved in **PostgreSQL**. **HDFS** acts as the high-availability layer, storing "Checkpoints" so the system can recover instantly if a service restarts.
...
> This isn't just a web app; it is a **Distributed Streaming System**. Even on a single machine, the components are fully decoupled — the simulator, Kafka broker, Spark engine, database, and API all run as independent processes. In a real-world scenario, each component could be scaled horizontally to separate physical servers without changing a single line of application code.
...
*   **Kafka**: A distributed log that captures raw user behavior. We use a single-broker configuration on `localhost` for high throughput.
...
- **Single-Machine Deployment**: The entire stack runs on one Ubuntu machine — no network configuration needed.
